...
//                                                            BookFont is Owls N' Ink by DubzyWubzyUbzy. Slightly modified.
...
using BookFont = QHash<int, QList<int>>;

BookFont gen_text_qhash() {
    BookFont h;
...
void draw_small_text_l(const BookFont &font, QPainter &painter, QPoint pos, QString text, QColor color) {
    int x = pos.x();
    int y = pos.y();
    int bearing_x = 2;
    int space_size = 5;
...
void draw_small_text_r(const BookFont &font, QPainter &painter, QPoint pos, QString text, QColor color) {
    int bearing_x = 2;
    int space_size = 5;
    int total_size = -1;
    painter.setPen(color);
